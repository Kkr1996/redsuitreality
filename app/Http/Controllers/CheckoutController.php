...
class CheckoutController extends Controller
{
    ...
    public function charge(Request $request)
    {
        ...
        $client_secret =  $stripe_setting->live_api_password;
        $stripe   = new \Stripe\StripeClient($client_secret);
        
        $customer = $stripe->customers->create([
            'description'     =>'RedSuit Reality Auction',
          ...
          $message_body);
           ...
           if(!$cart_row->isEmpty())
           {
             foreach($cart_row as $key=>$value)
             {
                  ...
                  $auction                     = Auction::find($product_id);
                  $product_price               = $auction->price;
                  $each_no_product_price[]     = $value->no_product * $product_price;
           ...
           //Invoice to the customer
            ...
            foreach($orders_details as $keyvalue)
            {
                ...
                foreach ($datass as $keyss => $valuess) {
                    $p_data = DB::table('auctions')->where('id',$valuess)->get();
    ...
    public function addToCart($productid)
    {
        ...
        $userid = user_id();
        $created_at       = date('Y-m-d h:i:s');
        $product_data     = Auction::find($productid);
        ...
        { 
          ...
          if(!$session_carts->isEmpty())
          {
            foreach($session_carts as $key=>$value)
            {
                ...
                $row->product_price = $product_data->price;
                $row->save();
     ...
     // }
        ...
        {
          $product_row       = Auction::find($productid);
    ...
    public function cart($removeid="")
    {
           ...
           if($carts)
            {
                ...
                {
                     $legal_text =  DB::table('auctions')->select('legal_disclaimer')->where('id',$product_values->product_id)->get();
        ...
        return  $this->response
                ...
                ->layout('home')
                ->view('cart')
                ->data(compact('page','sections','carts','legal_text_in_array'))
                ->output();
    }
    ...
    public function checkout()
    {
        ...
        return  $this->response
                ...
                ->layout('home')
                ->view('checkout')
                ->data(compact('page', 'sections','carts','stripe_setting','paypal_setting'))
                ->output();
    }
